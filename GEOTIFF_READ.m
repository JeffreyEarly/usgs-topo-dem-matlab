...
Tinfo        = imfinfo(name);
idx = 1;
info.samples = Tinfo(idx).Width;
info.lines   = Tinfo(idx).Height;
info.imsize  = Tinfo(idx).Offset;
info.bands   = Tinfo(idx).SamplesPerPixel;


sub = [1, info.samples, 1, info.lines];
%data_type = Tinfo.BitDepth/8;
data_type = Tinfo(idx).BitsPerSample(1)/8;
switch data_type
    case {1}
        format = 'uint8';
    case {2}
...
info.map_info.dx = Tinfo(idx).ModelPixelScaleTag(1);
info.map_info.dy = Tinfo(idx).ModelPixelScaleTag(2);
info.map_info.mapx = Tinfo(idx).ModelTiepointTag(4);
info.map_info.mapy = Tinfo(idx).ModelTiepointTag(5);
